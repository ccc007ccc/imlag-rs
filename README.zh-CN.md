...
- **两种触发模式**
  - **CFG 模式**：生成一组 `imlag_say_*.cfg`，patch `autoexec.cfg`，让 CS2 自己在按键触发时执行 `say "..."`。
  - **聊天模式**：模拟键盘打开聊天框、粘贴消息、按下回车。
...
| `bindKeys` / `teamBindKeys` | CFG 模式下绑定的全局 / 队内按键池（程序随机选一个） |
| `useCfgMode` | `true` = 走 CFG 轮替；`false` = 模拟键盘直接打字 |
